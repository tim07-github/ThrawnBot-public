...
public class Weather implements MessageCreateListener {
    /**
    ...
    public void onMessageCreate(MessageCreateEvent event) {
        ...
        if (message.getContent().startsWith("%wetter ") && !(message.getContent().equals("%wetter "))) {

            // Converts f.ex. umlauts to a more useful format
            byte[] stringByte = message.getContent().getBytes(StandardCharsets.UTF_8);
            ...
            if (!(content[1].isEmpty())) {
                ...
                String description = weatherActual.get("description").toString().replaceAll("\"", "");
                String temperature = main.get("temp").toString();
                String city = root.get("name").toString();
                String image = weatherActual.get("icon").toString().replaceAll("\"", "");
